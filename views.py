...
def edit_book_data(request,id):
    return HttpResponse(f"<label>A book with ID: {id} is edited...</label>")

def delete_student(request,roll):
    student = Students.objects.get(roll_number = roll)
    student.delete()
    return HttpResponse(f"<h2>Delete Student</h2><label>Student with Roll Number: {roll} is deleted...</label")


def delete_book(request,id):
    book = BookInstance.objects.get(pk = id)
    book.delete()
    return HttpResponse(f"<h2>Delete Book</h2><label>Book with ID: {id} is  deleted..</label>")

def return_issued_book(request,id):   
    obj=Book_Issue.objects.get(id=id)
    return HttpResponse(f"<h2>Return Issued Book</h2><label>Book <i>{obj.book_instance.book.book_title}</i> issued to <i>{obj.student.fullname}</i> is returned..</label>")

def edit_issued(request, id):
    obj=Book_Issue.objects.get(id=id)
    return HttpResponse(f"<h2>Edit Issued Book</h2><label>Book <i>{obj.book_instance.book.book_title}</i> issued to <i>{obj.student.fullname}</i>  is edited..</label>")
